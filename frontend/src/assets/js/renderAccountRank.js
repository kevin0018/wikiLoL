...
export function renderAccountRank(rankData, container) {
    ...
    const queueDisplayName = rankData.queueType === "RANKED_SOLO_5x5"
        ? "Clasificatoria solo/dúo"
        : "Clasificatoria flexible";

    // Handle tier display (including UNRANKED case)
    const tierDisplay = rankData.tier === "UNRANKED"
        ? "No clasificado"
    ...
    const winRate = totalGames > 0
        ? Math.round((rankData.wins / totalGames) * 100)
        : 0;
    ...
    icon.alt = `Icono rango ${tierDisplay}`;
    icon.className = [
    ...
    info.innerHTML = `
        ...
        <span class="text-gray-200 text-base sm:text-lg font-medium mb-1">
            ${rankData.tier === "UNRANKED" ? "-" : rankData.leaguePoints.toLocaleString() + " LP"}
        </span>
    `;
    ...
    if (rankData.tier === "UNRANKED" || totalGames === 0) {
        // For unranked or no games played
        stats.innerHTML = `
            <span class="text-gray-300 font-medium text-base sm:text-lg">
                0V <span class="opacity-60">0D</span>
            </span>
